...
const CreatePage = () => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      resetStore();
      toast.success("Pin published", "Your new pin is live.");
      navigate(`/pin/${data._id}`);
    },
    onError: (err) => {
      toast.error(
        "Could not publish pin",
  ...
  const handleSubmit = async () => {
    if (isEditing) {
      setIsEditing(false);
      toast.info("Edits applied", "Your design changes are ready to publish.");
    } else {
      if (!file) {
        toast.error("Choose a file", "Upload an image or video before publishing.");
        return;
      }
